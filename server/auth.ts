...
export function setupAuth(app: Express) {
  const sessionSettings: session.SessionOptions = {
    secret: process.env.SESSION_SECRET || "cyber-challenge-secret",
    resave: false,
  ...
  app.post("/api/register", async (req, res, next) => {
    try {
      const { username, password, groupCode, adminCode = "" } = req.body;
      ...
      const isAdmin = groupCode === "admin" && adminCode === "Godbless";
      
      // Verify admin group can only be created with correct admin code
      if (groupCode === "admin" && !isAdmin) {
